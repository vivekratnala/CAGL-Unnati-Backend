...
public interface LoanDtlsRepo extends CrudRepository<LoanDetails, BigDecimal> {
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE LoanDetails loan SET loan.coapplicantUpdateId = :coapplicantUpdateId "
			+ "WHERE loan.applicationId = :applicationId", nativeQuery = false)
	void updateCoapplicantUpdateId(@Param("applicationId") String applicationId,
			@Param("coapplicantUpdateId") String coapplicantUpdateId);
