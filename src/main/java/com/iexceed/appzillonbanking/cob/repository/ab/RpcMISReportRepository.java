...
public interface RpcMISReportRepository extends JpaRepository<ApplicationMaster, ApplicationMasterId> {
    ...
    @Query(value = "SELECT new com.iexceed.appzillonbanking.cob.payload.RpcMISReportPayload( " +
            " app.workitemNo , " +
            " CASE WHEN app.applicationStatus IN ('APPROVED', 'RPCPUSHBACK') THEN 'RPC Maker' WHEN app.applicationStatus IN ('PENDINGFORRPCVERIFICATION') THEN 'RPC Checker' " +
            ...
            " app.branchName, " +
            " 'NA', " +
            " CASE WHEN EXISTS(SELECT 1 FROM ApplicationWorkflow aw1 WHERE aw1.applicationId = app.applicationId AND aw1.applicationStatus IN ('PUSHBACK','RPCPUSHBACK','DBPUSHBACK','REJECTED')) " +
            ...
            " 'NA', " +
            " 'NA', " +
            " CASE WHEN app.applicationStatus IN ('APPROVED', 'RPCPUSHBACK') THEN 'Unnati_RPC  Maker' WHEN app.applicationStatus IN ('PENDINGFORRPCVERIFICATION') THEN 'Unnati_RPC  Checker' " +
