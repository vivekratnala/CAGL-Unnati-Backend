...
public class CommonService {
    ...
    public Mono<Object> sanctionApplicationMovement(FetchDeleteUserRequest fetchDeleteUserRequest, Header header,
        ...
        ResponseHeader responseHeader = new ResponseHeader();
        ResponseBody responseBody = new ResponseBody();
        if (!AppStatus.REJECTED.getValue().equalsIgnoreCase(fetchDeleteUserRequest.getRequestObj().getStatus()) &&
                !AppStatus.RPCVERIFIED.getValue().equalsIgnoreCase(fetchDeleteUserRequest.getRequestObj().getStatus())) {
            boolean isValidSanctionedAmount = loanDtlsRepo.eliglibleAmtMoreThanSanctionedAmt(fetchDeleteUserRequest.getRequestObj().getApplicationId());
            if (!isValidSanctionedAmount) {
                responseHeader.setResponseCode(ResponseCodes.FAILURE.getKey());
                response.setResponseHeader(responseHeader);
                responseBody.setResponseObj("Sanctioned amount cannot be greater than eligible amount.");
                response.setResponseBody(responseBody);
                return Mono.just(response);
            }
        }
                    ...
                    if (!msgNode.isMissingNode() && !msgNode.asText().trim().isEmpty()) {
                        ...
                        if (matcher.find()) {
                            ...
                            if(errorCustomerId.equalsIgnoreCase(updateCustomerId)){
                                logger.debug("dedupe Update completed for customerId: {}", updateCustomerId);
            ...
            try {
                ...
                if (apiRes.has(Constants.HEADER)) {
                    ...
                    if (ResponseCodes.SUCCESS.getValue().equalsIgnoreCase(status) || isBusinessRecordNotChanged) {
                        ...
                        loanDtlsRepo.updateCoapplicantUpdateId(applicationId, coApplicantupdateId);
